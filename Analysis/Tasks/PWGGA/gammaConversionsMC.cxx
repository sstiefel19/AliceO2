...
struct GammaConversionsMC {
  ...
  { 
    ...
    registry.fill(HIST("beforeCuts/hPhiRec_before"), thePhiV0Rec);
  ...
  template <typename TV0, typename TPOS, typename TNEG>
  void fillHistogramsAfterCuts(const TV0 &theV0, const TPOS &theTrackPos, const TNEG &theTrackNeg, float thePhiV0Rec, float thePsiPair, float theV0CosinePA)
  {
    registry.fill(HIST("IsPhotonSelected"), kPhotonOut);

    registry.fill(HIST("hPtRec"), theV0.pt());
    registry.fill(HIST("hEtaRec"), theV0.eta());
    registry.fill(HIST("hPhiRec"), thePhiV0Rec);
  ...
  void processTruePhotons(const TV0 &theV0, const TTRACK &theTrackPos, const TTRACK &theTrackNeg, const TMC &theMcParticles, float thePhiV0Rec)
  {
    ...
    if (theTrackPos.mcParticle().mother0() > -1 &&
      ...
      if (lMother.pdgCode() == 22) {
        ...
        registry.fill(HIST("resolutions/hPhiRes"), thePhiV0Rec - lMother.phi());
